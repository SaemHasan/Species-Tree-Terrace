    ...
    with open(inputFile) as fin:
        for line in fin: # for each gene tree
            ...
            for line_result in results_array:
                quartet_as_list = line_result.replace(";", "").replace("("," ").replace(")"," ").replace(",", " ").split()
                if len(set(quartet_as_list)) != 4:
                    continue
                for q in quartet_as_list:
                    all_species.add(q) 
            ...
            print("No of species: ", len(all_species)) 
